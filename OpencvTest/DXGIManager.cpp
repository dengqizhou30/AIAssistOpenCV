...
HRESULT DXGIManager::GetRectOutputBits(BYTE* pBits, RECT& rcDest)
{
	...
	if (vOutputs.size()>0)
	{
		...
		case DXGI_MODE_ROTATION_IDENTITY:
		{
			...
			{
				memcpy_s(pBuf + i * dwDestWidth * 4, dwStripe, map.pBits + (i + rcDest.top) * map.Pitch + rcDest.left * 4, dwStripe);
			}
		}
		break;
		case DXGI_MODE_ROTATION_ROTATE90:
		{
			...
			for (unsigned int j = 0; j < dwDestHeight; j++)
			{
				...
				{
					*(pDst + j * dwDestWidth + i) = *(pSrc + j + rcDest.top + dwMapPitchPixels * (rcDesktop.right - rcDesktop.left - i - 1 + rcDest.left));
				}
			}
		}
		break;
		case DXGI_MODE_ROTATION_ROTATE180:
		{
			...
			for (unsigned int j = 0; j < dwDestHeight; j++)
			{
				for (unsigned int i = 0; i < dwDestWidth; i++)
				{
					*(pDst + j * dwDestWidth + i) = *(pSrc + (rcDesktop.right - rcDesktop.left - i - 1 + rcDest.left) + dwMapPitchPixels * (rcDesktop.bottom - rcDesktop.top - j - 1 + rcDest.top));
				}
			}
		}
		break;
		case DXGI_MODE_ROTATION_ROTATE270:
		{
			...
			for (unsigned int j = 0; j < dwDestHeight; j++)
			{
				for (unsigned int i = 0; i < dwDestWidth; i++)
				{
					*(pDst + j * dwDestWidth + i) = *(pSrc + (rcDesktop.right - rcDesktop.left - j - 1 + +rcDest.top) + dwMapPitchPixels * (i + rcDest.left));
				}
			}
		}
		break;
